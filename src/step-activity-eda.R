...
get.merged.step.data.plot = function(step.activity.dataset) {
  ...
  axis(1, at=1:length(step.activity.dataset$step), labels = step.activity.dataset$step, cex.axis=0.8)
  #creating a legend to show what points correspond to what values
  legend("topright", legend = c("Participants","Completionists"), cex = 1, fill=c("blue","red"), text.font = 4)
  
  #plotting the percentage of completion for each step
  barplot(step.activity.dataset$step.complete.percent, axisnames = TRUE, names.arg = step.activity.dataset$step, cex.names=0.8,
  ...
  barplot(step.activity.dataset$time.to.complete.in.minutes, axisnames = TRUE, names.arg = step.activity.dataset$step, cex.names=0.8,
...
get.step.success.data.allruns.plot = function(allsteps) {
  ...
  axis(1, at=1:length(allsteps$step[allsteps$run==3]), labels = allsteps$step[allsteps$run==3], cex.axis=0.7)
  legend("bottomright", legend = c("run3","run4","run5","run6","run7"), cex = 0.70, fill=3:7, text.font = 4)
  ...
  plot(as.numeric(allsteps$step[allsteps$run==3]), allsteps$time.to.complete.in.minutes[allsteps$run==3],xaxt="n", type = "l", col=3, ylim = c(0,8000),
       xlab = "Step number", ylab = "Time (minutes)", main = "Time it took to complete each step on average")
  ...
  axis(1, at=1:length(allsteps$step[allsteps$run==3]), labels = allsteps$step[allsteps$run==3], cex.axis=0.7)
  legend("topleft", legend = c("run3","run4","run5","run6","run7"), cex = 0.70, fill=3:7, text.font = 4)
