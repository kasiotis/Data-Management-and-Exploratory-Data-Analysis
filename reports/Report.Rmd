...
This data set was recorded for all seven runs and it holds the responses of the learners about how they view themselves in regards to what archetype the feel like.
...
## Enrollments Dataset
```{r include=FALSE}
source("src/enrollments-eda.R", local = knitr::knit_global())
```

For the enrollments data set I had stated that my investigation would look at the Learner background and the Learner performance. Below we can see two sections that detail the analysis and its results about these two business goals. In order to make it easier for me to get a better picture of what happend in all runs, I have taken the values below by merging all runs and finding the sums of each variable. Essentially what that means is that the numbers of enrollments and graduations represent all seven runs, and so does the graduate percentage.
...
grad.general.data
```
The first thing we can see is that the learners did considerably better in terms of performance in run 1, with a graduation percentage of 12.5%. Whereas, in the rest of the runs only less that 4% completed the course. Additionally, the number of enrollments was decreasing as the runs went on. This means that not a lot people were signing up for the course.

### Enrollments and Graduation percentage based on age groups

```{r echo=FALSE, fig.align = "center"}
grad.age.data.merged[,1:4]
```
By observing the estimates for enrollments and graduations by age groups we see that the is a significant amount of people that did not provide any information about their age which may affect the reliability of the analysis. However, from the data that we have we see that the age groups with the most enrollments is that of 26-35. This is the group which seems to show the most interest in the course. Their performance however is mediocre, with just 8.8% graduating the course. The best performers were surprisingly the people who were over 65 years old, with an average across runs graduate percentage of 22.9%. When also looking at the rest of the age groups we can see that there may be a trend in our results. It seems that the age groups go down (the younger the learner), the smaller the percentage of graduates. 

### Enrollments and Graduation percentage based on Gender

```{r echo=FALSE, fig.align = "center"}
grad.gender.data.merged[,1:4]
```
...
```{r echo=FALSE, fig.align = "center"}
grad.education.data.merged[,1:4]
```
...
```{r include=FALSE}
source("src/leaving-survey-responses-eda.R", local = knitr::knit_global())
```
...
```{r echo=FALSE, fig.align = "center",  fig.width=11, fig.height=3.5}
...
```{r echo=FALSE, fig.align = "center", fig.width=11, fig.height=3}
...
## Step Activity dataset
```{r include=FALSE}
source("src/step-activity-eda.R", local = knitr::knit_global())
```
...
```{r include=FALSE}
source("src/weekly-sentiment-eda.R", local = knitr::knit_global())
```
...
## Question response dataset
```{r include=FALSE}
source("src/questions-eda.R", local = knitr::knit_global())
```
...
```{r echo=FALSE , fig.width=11, fig.height=2.8}
quiz.merged.plot()
```
The most successful quizzes seem 220 and 108, which had 80% and 65% of their responses correct. Whereas the the quizzes 311 and 316 which are both in Week 3 have very low success rates. This could be an indication that the material covers before the successful quizes was better understood by the learner, in contrast to that of week 3.

```{r echo=FALSE , fig.width=11, fig.height=3.5}
quiz.question.num.plot()
```
...
## Video Stats dataset
```{r include=FALSE}
source("src/video-stats-eda.R", local = knitr::knit_global())
```
