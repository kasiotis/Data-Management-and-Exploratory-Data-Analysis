...
For the enrollments data set I had stated that my investigation would look at the Learner background and the Learner performance. Below we can see two sections that detail the analysis and its results about these two business goals.
...
```{r echo=FALSE, fig.align = "center"}
grad.employment.data.merged[,1:4]
```

### Enrollments and Graduation percentage based on country of origin

In order to make it easier for me to get a better picture of what happend in all runs, I have taken the values below by merging all runs and finding the sums of each variable. Essentially what that means is that the numbers of enrollments and graduations represent all seven runs, and so does the graduate percentage.

In total there were learners from 200 different countries that enrolled in the course throughout the runs. The top 100 countries with the biggest number of enrollments can be seen below.
...
To understand the performance of the learners from all the countries I will investigate the graduate percentage of countries that were more that 20%

```{r fig.height=3 ,echo=FALSE}
country.grad.merged.plot.best
```
To get an ever better picture of the list of the top performing countries while also avoiding bias, I will set a minimum amount of enrollments to be 10 and bring the graduate percentage threshold down to 10%. This helps avoid presenting cases where the number of enrollments was really small and it happened that enough people graduated and brought the percentage up.
